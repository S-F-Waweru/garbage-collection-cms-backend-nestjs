...
import { CreateClientCreditDto, ClientCreditResponseDto } from "../dto/client-credit.dto";
...
export class CreateClientCreditUseCase {
    ...
    async execute(dto: CreateClientCreditDto): Promise<ClientCreditResponseDto> {
        ...
        const existingCredit = await this.creditRepository.findByClientId(dto.clientId);
        if (existingCredit) {
            throw new BadRequestException(`Client credit already exists for client ${dto.clientId}`);
        }
