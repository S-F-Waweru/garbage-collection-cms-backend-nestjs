...
export class ClientCreditRepository implements ICreditClientRepository {
    ...
    async incrementBalance(clientId: string, amount: number): Promise<ClientCredit> {
        const credit = await this.findByClientId(clientId);
        
        if (!credit) {
            throw new NotFoundException(`Client credit not found for client ${clientId}`);
        }
    ...
    async decrementBalance(clientId: string, amount: number): Promise<ClientCredit> {
        const credit = await this.findByClientId(clientId);
    ...
    async updateBalance(clientId: string, newBalance: number): Promise<ClientCredit> {
        const credit = await this.findByClientId(clientId);
